...
function getOxc() {
  ...
  try {
    // Use webpackIgnore to prevent Next.js from bundling the native module.
    // In Node.js (tests): loads the Rust-based native parser.
    // In browser: require fails, falls back to regex.
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    oxc = require(/* webpackIgnore: true */ "oxc-parser") as typeof oxc;
    return oxc;
  } catch {
    oxcLoadFailed = true;
    return null;
